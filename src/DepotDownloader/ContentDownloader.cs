...
namespace DepotDownloader
{
	...
	static class ContentDownloader
	{
		...
		private static Steam3Session steam3 = new Steam3Session(new SteamUser.LogOnDetails());
		...
		public static async Task DownloadPubfileAsync(uint appId, ulong publishedFileId)
		{
			...
			{
				await DownloadAppAsync(appId, new List<(uint, ulong)> { (appId, details.hcontent_file) }, DEFAULT_BRANCH, null, null, null, false, true, null);
			}
			else
			{
		...
		public static async Task DownloadUGCAsync(uint appId, ulong ugcId)
		{
			...
			{
				await DownloadAppAsync(appId, [(appId, ugcId)], DEFAULT_BRANCH, null, null, null, false, true, null);
			}
		}
		...
		public static async Task DownloadAppAsync(uint appId, List<(uint depotId, ulong manifestId)> depotManifestIds, string branch, string os, string arch, string language, bool lv, bool isUgc, Game game)
		{
			...
			else
			{
				...
				{
					Console.WriteLine("DownloadAppAsync: No depots found");
		...
		{
			Console.WriteLine("ProcessDepotManifestAndFiles: Starting");
			...
			GlobalDownloadCounter downloadCounter, DepotFilesData depotFilesData, HashSet<string> allFileNamesAllDepots, Game game)
		{
			...
			Console.WriteLine("DownloadSteam3AsyncDepotFiles: parallel options created");
			...
			await Parallel.ForEachAsync(networkChunkQueue, parallelOptions, async (q, cancellationToken) =>
			{
				await DownloadSteam3AsyncDepotFileChunk(
					...
					q.fileData, q.fileStreamData, q.chunk, game
				);
			});
